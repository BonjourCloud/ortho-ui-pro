        ...
        <div className="container">
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <p className="text-primary-foreground/80 leading-relaxed mb-4">{doctorProfile.title}</p>
              <p className="text-primary-foreground/70 leading-relaxed">{doctorProfile.fullBio}</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.2 }}>
              <img src={heroImage} alt={doctorProfile.name} className="rounded-2xl shadow-2xl w-full object-cover aspect-[4/3]" />
            </motion.div>
          </div>
        </div>
      </section>
        ...
        <div className="container max-w-3xl text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {doctorProfile.memberships.map((m, i) => (
              <span key={i} className="bg-card rounded-full px-5 py-2 text-sm font-medium text-foreground border shadow-sm">
                {m}
              </span>
            ))}
          </div>
