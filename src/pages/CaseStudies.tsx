      ...
      <section className="py-12 md:py-20">
        <div className="container">
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">No case studies yet.</div>
          ) : (
                  ...
                  <Link to={`/case-studies/${cs.slug}`} className="block bg-card rounded-xl border hover:shadow-lg transition-all overflow-hidden group h-full">
                    <div className="p-6">
                      <div className="flex items-center gap-2 mb-3">
                        <span className="text-xs font-medium text-accent bg-accent/10 rounded-full px-3 py-1">{cs.surgery_type}</span>
                        <span className="text-xs text-muted-foreground">{cs.patient_initials} • Age {cs.age}</span>
                      </div>
                      ...
                      <p className="text-sm text-muted-foreground mb-2">{cs.procedure_name}</p>
                      <p className="text-sm text-muted-foreground leading-relaxed mb-4 line-clamp-2">{cs.outcome_summary}</p>
                      <div className="grid grid-cols-2 gap-3 p-3 bg-secondary/50 rounded-lg mb-4">
                        <div className="text-center">
                          ...
                          <div className="font-display text-xl font-bold text-destructive flex items-center justify-center gap-1">
                            {cs.pain_score_pre}/10 <TrendingUp size={14} />
                          </div>
                        </div>
                        <div className="text-center">
                          ...
                          <div className="font-display text-xl font-bold text-primary flex items-center justify-center gap-1">
                            {cs.pain_score_post}/10 <TrendingDown size={14} />
                          </div>
                        </div>
                      </div>
